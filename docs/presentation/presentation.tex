...
\begin{frame}{Encoder and Decoder Reference Implementation}
	\begin{itemize}
		\item Written in Rust.
		      \note[item]{systems level programming language similar to C and C++, strong memory safety guarantees}
		\item Extensively Tested.
		      \note[item]{100 percent code coverage}
		      \note[item]{not going to talk a ton about the reference implementation}
		\item Majority of time was spent on this implementation.
	\end{itemize}
\end{frame}
...
\begin{frame}{Performance Test}
	\begin{columns}
		...
		\end{column}
		\begin{column}{0.5\textwidth}
				\item For each data format the client:
				      \begin{itemize}
