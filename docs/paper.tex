...
XbfVec was implemented as native dynamic array of XbfType values, and a reference counted pointer to the metadata of its inner type. In order to ensure XbfVec remains homogenous, it requires the metadata to be passed in at construction time. If all of the values given to the constructor do not have the same metadata as the metadata passed in, the constructor returns an
